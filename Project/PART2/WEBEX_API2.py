...
current_access_token = "ZDE0YWRjMjgtMWRkMy00MjgyLTkxYmEtOTA2YjAxODNkMjIwYmRjNDY5YmYtZTdj_P0A1_f47c3c72-ce4d-4f2f-9d29-02816c42a704" ### add your own bearer token
...
def main2(): # using rest api
    ...
    for rec in groups_struc["groups"]:
        ...
        if groups_struc["title"] != None:  ### avoid errors if room title unknown

            res_space = requests.post(url, headers=headers, json=groups_struc)
...
for email in groups_struc["group"]["members"]:
            api.memberships.create(groups_struc.id, personEmail=email["email"])
